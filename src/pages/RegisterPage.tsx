...
export default function RegisterPage() {
  ...
  const { lang, setOtpEmail, setOtpPassword, setOtpRole, setOtpName } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!name || !email || !password) { setError(t('auth.errors.fieldRequired')); return; }
    setLoading(true);
    try {
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }} className="relative w-full max-w-[440px]">
        <div className="bg-white rounded-[2.25rem] border border-slate-100/90 shadow-[0_32px_64px_-12px_rgba(30,41,59,0.14)] overflow-hidden">
          ...
          <div className="p-8 sm:p-10">
            ...
            {/* OTP hint */}
            <div className="mb-6 rounded-2xl bg-emerald-50 border border-emerald-200/60 p-3.5 text-xs text-emerald-800">
              <p className="font-bold flex items-center gap-2">
                <ShieldCheck className="h-3.5 w-3.5" />
                {lang === 'ka' ? 'Mock OTP კოდი შემდეგ ეტაპზე:' : 'Mock OTP code for next step:'}
                <span className="font-mono font-black text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-lg">{MOCK_OTP_CODE}</span>
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Name */}
              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block">{t('auth.fullName')}</label>
                <div className="relative"><UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" /><input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('auth.namePlaceholder')} required className="w-full rounded-2xl border border-slate-200/80 bg-slate-50/50 py-3.5 pl-11 pr-4 text-xs font-medium text-slate-900 placeholder-slate-400 focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" /></div>
              </div>
              {/* Email */}
              <div className="space-y-1.5">
