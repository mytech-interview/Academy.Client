...
export default function VideosTab({ videos, searchQuery, onDelete }: VideosTabProps) {
  const filtered = useMemo(() => {
    ...
    return videos.filter(
      (v) => v.category.toLowerCase().includes(q) || v.title.toLowerCase().includes(q) || v.instructor.toLowerCase().includes(q)
    );
  }, [videos, searchQuery]);
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        <EmptyState message={searchQuery.trim() ? `ვიდეო ვერ მოიძებნა: "${searchQuery}"` : 'ვიდეოები ჯერ არ დამატებულა'} />
      ) : (
          ...
          {filtered.map((video) => (
            ...
            >
              <div className="relative h-44 overflow-hidden bg-slate-100">
                <img src={video.image} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
              ...
              <div className="p-5 flex flex-col flex-1 justify-between space-y-3">
                <div>
                  ...
                  <h3 className="font-extrabold text-slate-800 text-sm leading-snug line-clamp-2">{video.title}</h3>
