...
import { mockValidateOtp } from '../api/mockAuthApi';
...
export default function OtpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await mockValidateOtp(otpEmail, code);
      ...
      >
        <div className="bg-white rounded-[2.25rem] border border-slate-100/90 shadow-[0_32px_64px_-12px_rgba(30,41,59,0.14)] overflow-hidden">
          ...
          <div className="p-8 sm:p-10">
            ...
            {/* Mock OTP hint */}
            <div className="mb-6 rounded-2xl bg-amber-50 border border-amber-200/60 p-4 text-center">
              <p className="text-xs font-bold text-amber-700 mb-2">
                🧪 {lang === 'ka' ? 'Mock OTP კოდი:' : lang === 'ru' ? 'Mock OTP код:' : 'Mock OTP code:'}
              </p>
              <div className="flex items-center justify-center gap-2">
                {MOCK_OTP_CODE.split('').map((d, i) => (
                  <span
                    key={i}
                    className="h-9 w-8 flex items-center justify-center rounded-xl bg-amber-100 border border-amber-300 text-base font-black text-amber-800 font-mono"
                  >
                    {d}
                  </span>
                ))}
              ...
              <p className="text-[10px] text-amber-600 mt-2 font-medium">
                {lang === 'ka' ? 'ჩაწერეთ ქვემოთ ან ჩასვით Ctrl+V' : lang === 'ru' ? 'Введите ниже или вставьте Ctrl+V' : 'Type below or paste with Ctrl+V'}
              </p>
            </div>
