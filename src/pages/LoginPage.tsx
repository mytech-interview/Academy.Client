...
import { mockCreateOtp } from '../api/mockAuthApi';
...
export default function LoginPage() {
  ...
  const {
    registeredUsers, lang,
    setOtpMode, setOtpPendingUser,
    setOtpEmail, setOtpPassword, setOtpRole, setOtpName,
  } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 1. Find the user in registered list
      const found = registeredUsers.find(
        (u) => u.email.toLowerCase() === email.toLowerCase()
      );
      if (!found) { setError(t('auth.errors.userNotFound')); return; }

      // 2. Send OTP (mock)
      await mockCreateOtp(email);

      // 3. Store state for OtpPage
      setOtpMode('login');
      setOtpPendingUser(found);
      setOtpEmail(email.toLowerCase());
      setOtpPassword(password);
      setOtpRole(found.role as 'student' | 'teacher');
      setOtpName(found.name);
      ...
      >
        <div className="bg-white rounded-[2.25rem] border border-slate-100/90 shadow-[0_32px_64px_-12px_rgba(30,41,59,0.14)] overflow-hidden">
          ...
          <div className="p-8 sm:p-10">
            ...
            {/* Demo hint */}
            <div className="mb-6 rounded-2xl bg-amber-50 border border-amber-200/60 p-4 text-xs text-amber-800">
              <p className="font-bold mb-1.5">
                🧪 {lang === 'ka' ? 'ტესტის ანგარიშები:' : lang === 'ru' ? 'Тестовые аккаунты:' : 'Demo accounts:'}
              </p>
              <div className="space-y-1 font-mono text-[11px]">
                <p>👩‍🏫 <strong>m.beridze@academy.ge</strong> — {lang === 'ka' ? 'მასწავლებელი' : 'Teacher'}</p>
                <p>🎓 <strong>demo.student@academy.ge</strong> — {lang === 'ka' ? 'სტუდენტი' : 'Student'}</p>
                <p className="text-amber-600 mt-1">
                  {lang === 'ka' ? 'პაროლი: ნებისმიერი' : lang === 'ru' ? 'Пароль: любой' : 'Password: anything'}
                </p>
                <p className="text-amber-700 font-bold mt-1.5">
                  🔐 OTP: <span className="font-mono tracking-widest">{MOCK_OTP_CODE}</span>
                </p>
              </div>
            </div>
