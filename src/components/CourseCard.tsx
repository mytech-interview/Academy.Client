...
const formatDate = (d: Date) => {
  ...
  const statusMeta = status && {
    ...
    ongoing: { text: t('courseCard.statusOngoing', 'Ongoing'), cls: 'bg-emerald-100 text-emerald-800 border-emerald-300' },
    completed: { text: t('courseCard.statusCompleted', 'Completed'), cls: 'bg-slate-200 text-slate-700 border-slate-300' },
  }[status];
        ...
        <div className="flex items-center gap-1.5 text-[11px] font-mono font-semibold text-indigo-600 uppercase tracking-widest mb-1.5">
          ...
          {durationHours > 0 && (
            <>
              ...
              <span>{t('courseCard.hours', { count: durationHours, defaultValue: `${durationHours} hours` })}</span>
            </>
          )}
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {isLoggedIn && userRole === 'teacher' ? (
              ...
              >
                ...
                <span>{t('courseCard.statusActive', 'Active')}</span>
              </button>
            ) : (
              <button
              ...
              >
                ...
                <span>{isEnrolling ? t('courseCard.btnEnrolling', 'Enrolling...') : t('courseCard.btnEnroll', 'Enroll')}</span>
              </button>
            )}
          </div>
        </div>
