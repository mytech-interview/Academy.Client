  ...
  return (
    <>
      <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-base font-black text-slate-800">{t('projectsTab.title')}</h2>
          <p className="text-xs text-slate-400 mt-0.5">{t('projectsTab.subtitle')}</p>
        </div>
        <button className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2 shadow-md shadow-purple-200 transition">
          <Plus className="w-4 h-4" /> {t('projectsTab.addBtn')}
        </button>
      </div>

      {projects.length === 0 ? (
        <EmptyState message={t('projectsTab.empty')} />
      ) : (
              ...
              <div className="p-5 flex flex-col flex-1 justify-between space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    {t('projectsTab.author', { name: project.author })}
                  </p>
                </div>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center gap-2">
                  ...
                  <button
                    ...
                    title={t('projectsTab.deleteTitle')}
                  >
